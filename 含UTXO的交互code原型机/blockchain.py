...
class Blockchain:
    def __init__(self, address = hashlib.sha256((str(np.random.randint(1,1000))).encode()).hexdigest()):
        ...
        self.NoneAddress = "NoneAddress" #hashlib.sha256("NoneAddress".encode()).hexdigest()
    ...
    def mine(self):
        ...
        ###coinbase
        _Tx=Transaction.create_coinbase_Tx(noneaddresss= self.NoneAddress, address_to = self.node_address, pub_key_hash='')     ###coinbase Transaction
    ...
    def transfer(self, address_payer, address_receiver, total_value):
        ...
        if sum_value<total_value:
            ...
            while not tmp_prepayer_queue.empty():
                ...
                tmp_value=tmp_value_queue.get()
                tmp_Txid=tmp_Txid_queue.get()
                vins.append(tx_input(tmp_Txid, address_receiver, ''))  ### should implement pub_key later : pub_key = addressOne.signature
                vouts.append(tx_output(tmp_value, ''))  ### should implement pub_key_hash later
