...
"""区块链结构原型包含以下基本内容。
a) 索引：当前区块的高度。b) 时间戳。c) 上一个区块哈希。d) 当前区块哈希。e) 难度：区块哈希开头的位数，动态变化。
f) 随机数：用于计算区块哈希的随机数。g) 交易的Merkle根。h) 数据：交易。
...
class BlockHeader:
    ...
    def calculate_blockheader_hash(self):
        ...
        # 经过两次 SHA-256 计算生成，返回str
        return hashlib.sha256(hashlib.sha256(block_header_string.encode()).digest()).hexdigest()
...
class Block:
    ...
    def __init__(self, index=0, timestamp=0, prev_hash="0", difficulty=0, merkle_root=0, nonce=0, txs_data=None):
        ...
        if len(txs_data) > TRANSACTION_LIMIT:
            logging.warning(f"Block contains too many Transactions(more than {TRANSACTION_LIMIT}) "
                            f"and may not be accept by validators.")
        ...
        self.block_hash = hashlib.sha256(hashlib.sha256(block_hash_data.encode()).digest()).hexdigest()  # 返回值str
    ...
    def validate_block(self):
        ...
        if block_hash_new != self.block_hash:
            return False
        else:
            return True
...
class Blockchain:
    ...
    def __init__(self, p2p_port, db=None):
        ...
        if db:
            ...
            try:
                ...
                self.db = None
                logging.error(f"区块链无法连接到LevelDB，{str(e)}")
    ...
    def validate_blockchain(one_Blockchain) -> bool:
        ...
        1.区块哈希验证：调用每个区块的validate_block()方法，确保区块数据未被篡改。
        2.工作量证明（PoW）验证：检查区块哈希是否满足该区块难度要求（前导零数量）。
        3.Merkle根验证：重新计算交易的Merkle根，确保交易数据完整性。
        4.链式结构验证
        5.检查前哈希（prev_hash）的连续性
        6.检查索引号的递增性
        7.创世区块前哈希必须为"0"
        8.高度一致性验证：确保区块链长度与高度值一致（高度=长度）。
        """
        ...
        for block in one_Blockchain.blockchain:
            ...
            calculated_merkle = MiningModule.calculate_merkle_root(txids)
            ...
            if previous_block:
                ...
                if block.header.index != previous_block.header.index + 1:
                    logging.error(f"区块{block.header.index} 索引不连续，当前索引 {block.header.index}")
                    return False
            else:
                # 创世区块特殊检查
                if not all(c == '0' for c in block.header.prev_hash):
                    logging.error(f"区块链创世区块前哈希不为0 {block.header.prev_hash}")
                    return False
