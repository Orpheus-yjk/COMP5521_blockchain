...
class GenerateKeysUtils:
    ...
    """
    @staticmethod
    def generate_arbitrary_random_number(bit_num):
        """生成任意字节随机数"""
        return os.urandom(bit_num)
    ...
    def randomly_generate_privateKey_and_publicKey_and_publicKeyHash(output_info=False):
        ...
        if output_info:
            logging.info(f"私钥 (hex): {priv_key.hex()}")

        # 2. 生成压缩公钥（33字节）
        pub_key = GenerateKeysUtils.private_key_to_public_key(priv_key)
        if output_info:
            logging.info(f"公钥 (hex): {pub_key.hex()}")
...
class VerifyHashAndSignatureUtils(GenerateKeysUtils, SignMessageUtils):
    ...
    @staticmethod
    def unlock_p2pkh_script(_locking_script: str, public_key: bytes, signature: bytes) -> str:
        """解锁P2PKH脚本。返回: (签名, 公钥)
        ...
        parse_script = _locking_script.split()  # 脚本解析过于简单：当前代码用 split() 简单分割锁定脚本，但真实的比特币脚本是二进制格式，需要更严谨的解析逻辑。
        if parse_script[0] != "OP_DUP" or parse_script[1] != "OP_HASH160" or \
                parse_script[3] != "OP_EQUALVERIFY" or parse_script[4] != "OP_CHECKSIG" or len(parse_script) != 5:
            logging.error("锁定脚本格式错误，无法解析！")
        ...
        if rehash_pubkey != pubkey_hash_loaded:
            logging.error("公钥哈希不匹配！")
            return "Unlock Script Fail!"

        # 构建解锁脚本: <sig> <pubKey>
        _unlocking_script = ' '.join([signature.hex(), public_key.hex()])
        return _unlocking_script
    ...
    def verify_signature(public_key: bytes, signature: bytes, message: bytes) -> bool:
        ...
        if len(public_key) not in [33, 65]:  # 压缩公钥33字节，非压缩65字节
            logging.error("无效的公钥长度")
            return False

        if len(signature) != 64:  # ECDSA签名通常为64字节
            logging.error("无效的签名长度")
            return False
        ...
        except (ecdsa.BadSignatureError, ValueError) as _err:
            return False
