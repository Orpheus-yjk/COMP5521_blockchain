...
INPUT_SIGNATURE_PLACEHOLDER = b'1143f5f4f38b526d9202a14dfc1db54cf9270d5762d88d13d704e5bdad' \
                              b'4262b10caf7a44d344e8b041bcd753b4e7cbca0fb6d1dc93bcb2440fa376785b8c972f'.hex()
...
class Transaction(CoinbaseScript, StandardTransactionScript, VerifyHashAndSignatureUtils):
    ...
    def create_coinbase_Tx(cls, block_height, miner_address, mining_reward):
        ...
        # 解析
        tx_input = txInput(Txid, Tx_script["vins"][0]["referid"], CoinbaseScript.COINBASE_PUBLKEY,
                           Tx_script["vins"][0]["scriptSig"])  # 签名人：超级节点
    ...
    @staticmethod
    def payer_sign(private_key: bytes, receiver_address: str, amount: int, Tx_data: dict, txid: str, referid: int) \
            -> Tuple[txInput, txOutput, bytes]:
        """资金发送者签名得到signature，并且生成资金流返回"""
        # 处理错误
        if "version" not in Tx_data.keys() or "locktime" not in Tx_data.keys() or "vins" not in Tx_data.keys() \
                or "vouts" not in Tx_data.keys():
        ...
        tx_input = txInput(
            txid=txid,
            referid=referid,
            pubkey=VerifyHashAndSignatureUtils.private_key_to_public_key(private_key),
            signature=Sig.hex()
        )
        tx_output = txOutput(
            value=amount,
            pubkey_hash=receiver_address
        )
    ...
    def verify_transaction(locking_script: str, public_key: bytes, signature: bytes, Tx_data: dict):
        ...
        if unlocking_script == "Unlock Script Fail!":
            return False
