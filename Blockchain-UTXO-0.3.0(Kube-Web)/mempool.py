...
from db_module import RedisModule, MongoDBModule
...
class UTXOManager:
    ...
    def __init__(self, p2p_port):
        ...
        # self.load_utxos()  # 使用MongoDB存储UTXO，但初始化不加载，等待client的主动操作。因为client可能选择丢弃MongoDB中的数据
...
class Mempool:
    ...
    def flush_current_size(self):
        """从self.transactions更新current_size的值"""
        self.current_size = 0
        for _, _tx in self.transactions.items():
            self.current_size += _tx.calculate_raw_size()
    ...
    def validate_transaction(self, tx: Transaction) -> bool:
        """完整交易验证流程"""
        # logging.info("\n--- 交易验证详细信息 ---")
        # logging.info(f"交易ID: {tx.Txid}")
        # logging.info("输入详情:")
        # for i, vin in enumerate(tx.vins):
        #     logging.info(f"输入{i}:")
        #     logging.info(f"  txid: {vin.txid}")
        #     logging.info(f"  referid: {vin.referid}")
        #     logging.info(f"  pubkey: {vin.pubkey.hex() if isinstance(vin.pubkey, bytes) else vin.pubkey}")
        #     logging.info(f"  signature: {vin.signature.hex() if isinstance(vin.signature, bytes) else vin.signature}")
        ...
        for vin in tx.vins:
            ...
            if self.utxo_monitor.is_spent(vin.txid, vin.referid):
                logging.error(f"双花检测: {vin.txid}:{vin.referid}")
                return False
            elif vin.txid not in self.utxo_monitor.utxos.keys() and not is_coinbase:
                logging.error(f"未收到有关交易的任何信息，故无法使用: {vin.txid}:{vin.referid}")
                return False
            ...
            # 验证引用是否正确
            if not utxo:
                logging.error(f"未引用正确的tx在tx_input中: {vin.txid}:{vin.referid}")
                return False
            ...
            if not VerifyHashAndSignatureUtils.verify_signature(
                ...
                logging.error("\n验证失败详细信息:")
                logging.error("Public Key:", public_key.hex())
                logging.error("Signature:", signature.hex())
                logging.error("交易message:", message.hex(), "\n")
                return False
            total_input += utxo[0]
        ...
        if total_output + tx.fee > total_input:
            logging.error("所需金额（含手续费）超过输入金额。转账终止")
            return False
    ...
    def add_transaction(self, tx: Transaction) -> bool:
        ...
        if not self.validate_transaction(tx):
            logging.error(f"交易验证失败: txid: {tx.Txid}")
            return False
    ...
    def remove_transaction(self, txid: str):
        ...
        if txid in self.transactions:
            ...
            self.remove_transactions_from_DB()  # 去除交易 O(n)
            self._save_transactions()  # 添加新交易 O(n)
        ...
        for tx in block_transactions:
            ...
            for idx, vout in enumerate(tx.vouts):
                self.utxo_monitor.add_utxo(tx.Txid, idx, vout.value, vout.pubkey_hash,
                                           "OP_DUP OP_HASH160 {} OP_EQUALVERIFY OP_CHECKSIG".format(vout.pubkey_hash))
    ...
    def rebuild_utxo_from_all_blocks(self, all_blocks):
        ...
        with self.utxo_monitor.lock:  # 加锁
            ...
            # can_flush_db == False 是增量构建，如果中间经过了竞争挖矿导致的换链，会在LevelDB中保留原来区块链废弃的块，一般不用
            can_flush_db = True
            if can_flush_db:
    ...
    def __repr__(self):
        return f"Mempool(交易数={len(self.transactions)}, " \
               f"占用内存={self.current_size}/{self.max_size} bytes({self.current_size / self.max_size:.5%}))"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"手续费最高的交易: {top_txs[0].Txid}:{top_txs[0].fee}")
    except:
    ...
    mempool.transactions = {}  # 删除所有tx（如果本地有数据可注释掉这两行）
    mempool.remove_transactions_from_DB()
